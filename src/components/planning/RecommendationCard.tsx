...
export function RecommendationCard({ visibility, minAltitudeDeg, timezone }: RecommendationCardProps) {
  ...
  return (
    <>
      <div className="bg-space-surface border border-space-border rounded-xl flex flex-col overflow-hidden hover:border-space-border-light transition-colors">
        {/* Banner: object thumbnail with altitude sparkline overlay at the bottom. */}
        <ObjectThumbnail
          catalogId={visibility.catalog_id}
          name={visibility.name}
          type={visibility.type}
          aspect="16/9"
        >
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-black/70 via-black/30 to-transparent" />
          <div className="pointer-events-auto absolute inset-x-0 bottom-0 px-2 pb-1.5">
            <AltitudeSparkline
              ...
              height={48}
              variant="over-image"
              riseTime={visibility.rise_time}
              transitTime={visibility.transit_time}
              setTime={visibility.set_time}
              timezone={timezone}
    ...
    >
      <div className="space-y-4">
        ...
        >
          ...
          <div className="pointer-events-auto absolute inset-x-0 bottom-0 px-3 pb-2">
            <AltitudeSparkline
              curve={visibility.altitude_curve}
              minAltitudeDeg={minAltitudeDeg}
              width={600}
        ...
        <p className="text-xs text-text-muted -mt-2">
          Altitude through the night (dashed line at min {minAltitudeDeg}°). Hover the markers
          for rise, transit and set times.
        </p>
